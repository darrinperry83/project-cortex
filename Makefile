...
.PHONY: help db-up db-up-sandbox db-down db-logs db-psql db-shell db-backup db-restore
...
db-psql:
	...
	@docker exec -it $(CONTAINER) psql -U cortex -d $(DB)
...
db-backup:
	...
	docker exec $(CONTAINER) sh -c "pg_dump -U cortex -d $(DB) -f /backups/$(DB)-$${DATE}.sql"; \
...
db-restore:
	...
	docker exec -i $(CONTAINER) psql -U cortex -d $(DB) -f /backups/$$BASENAME; \
